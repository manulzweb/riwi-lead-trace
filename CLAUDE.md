...
6. **Integridad del instrumento (ADMIN-02):** las preguntas solo se editan **con periodo cerrado**. El admin **sí puede** ajustar el **peso** (`weight_percent`) de cada pregunta de escala - es la única ponderación que existe - pero **no puede tocar** `category` ni `input_type`. Los pesos de las preguntas de escala **activas de un mismo `form` deben sumar exactamente 100** (validado en `question_service` antes de guardar; si no suma, se rechaza con `422`/`409`). Las respuestas históricas conservan su pregunta y su peso original.
...
| PATCH | `/questions/:id` | editar texto de una pregunta | **solo periodo cerrado** |
